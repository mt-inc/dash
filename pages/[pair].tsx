...
class Home extends React.Component<Props, State> {
  ...
  async loadPair(p?: string) {
    ...
    const res = await fetch(`https://mt.raptom.com.ua/api/data/${pair}`, { method: 'get' }).then((res) => res.json());
    this.setState((prev) => ({ ...prev, open: false, data: res as Result, current: 0 }));
  }
  ...
  render() {
    ...
    if (props && props.result.length > 0) {
      const cols: Cols[] = Object.keys(props.result[0].data[c].results[0]).map((key, ind) => ({
        field: key,
        sortable: key !== 'Опції',
        headerName: key,
        width: ind === 0 ? 250 : 200,
        renderCell:
          key === 'Опції'
            ? (params: any) => (
                <div>
                  {params.value.split('  ').map((item: string, ind: number) => (
        ...
        hide: ['Активна позиція', '% збиткових позицій', 'Середній прибуток', 'Середній збиток', 'Очікування'].includes(
          key,
        ),
        valueFormatter: ['Максимальне падіння', '% збиткових позицій', '% прибуткових позицій'].includes(key)
          ? (params) => (params.value ? `${params.value}%` : undefined)
          : undefined,
      }));
      cols.splice(0, 0, { field: 'id', headerName: '#', width: 70, filterable: false });
      const res = props.result[0].data[c].results.sort(
        (a, b) => a.Прибуток + a['Активна позиція'] - (b.Прибуток + b['Активна позиція']),
      );
      const rows = res.reverse().map((res, ind) => ({
        ...res,
        'Максимальне падіння': res['Максимальне падіння'] === 'н/д' ? null : parseFloat(res['Максимальне падіння']),
      ...
      return (
        <ThemeProvider theme={this.getTheme()}>
          ...
          <AppBar position="sticky" style={{ zIndex: 100000 }} color={this.state.theme ? 'secondary' : 'primary'}>
            <Toolbar>
              <IconButton edge="start" color="inherit" aria-label="menu" onClick={this.toggle}>
                <MenuIcon />
              </IconButton>
